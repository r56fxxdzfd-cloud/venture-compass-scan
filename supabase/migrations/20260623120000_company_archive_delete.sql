...
-- Roda como definer (ignora RLS) e dentro de uma transação: se algo falhar,
-- nada é removido. Ordem dos filhos do mais profundo para o mais raso.
...
BEGIN
  ...
  -- Founders e dependências (referem founder_assessment_id)
  ...
  -- Diagnósticos (answers / assessment_red_flags caem por cascade do assessment)
  ...
  -- Intake: preserva o registro do envio, apenas desvincula
  UPDATE public.intake_submissions SET company_id = NULL WHERE company_id = p_company_id;

  -- advisor_assignments / action_items / meeting_logs caem por ON DELETE CASCADE
  DELETE FROM public.companies WHERE id = p_company_id;
END;
$$;

-- 4) Restringe DELETE direto na tabela a Super Admin (defesa em profundidade).
